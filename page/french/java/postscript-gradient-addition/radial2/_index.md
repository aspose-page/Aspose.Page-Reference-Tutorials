---
date: 2026-02-13
description: Apprenez à remplir une forme avec un dégradé et à dessiner un cercle
  avec un dégradé en Java PostScript à l'aide d'Aspose.Page. Guide étape par étape
  avec du code et des astuces.
linktitle: Java PostScript Radial Gradient with Aspose.Page
second_title: Aspose.Page Java API
title: 'Remplir la forme avec un dégradé : Exemple Java PostScript radial'
...
# Remplir une forme avec un dégradé : Exemple radial Java PostScript

## Introduction
Dans ce tutoriel, vous apprendrez à **remplir une forme avec un dégradé** en créant un exemple de dégradé radial pour un document PostScript à l’aide d’Aspose.Page pour Java. Nous parcourrons chaque étape — de la configuration du projet à la génération d’un cercle rempli d’un dégradé radial lisse — afin que vous puissiez ajouter instantanément des graphiques accrocheurs à vos applications Java.

## Réponses rapides
- **Quel est le résultat de ce tutoriel ?** Un fichier PostScript (`.ps`) contenant un cercle rempli d’un dégradé radial.  
...
- **Ai‑je besoin d’une licence ?** Une licence temporaire ou complète est requise pour une utilisation en production ; une version d’essai gratuite suffit pour le développement.  
...
## Comment remplir une forme avec un dégradé dans PostScript
Avant de plonger dans le code, clarifions pourquoi « remplir une forme avec un dégradé » est important. L’utilisation de dégradés confère à vos graphiques un aspect professionnel et tridimensionnel sans recourir à des images raster. Avec Aspose.Page, vous pouvez appliquer la même logique de dégradé à n’importe quelle forme vectorielle — cercles, rectangles ou chemins personnalisés — sur tous les formats de sortie pris en charge (PostScript, PDF, SVG).
...
- **Sortie de haute qualité** – prend en charge l’anti‑aliasing et le contrôle de l’espace colorimétrique.

## Prérequis
Avant de commencer, assurez‑vous de disposer de :

- Une connaissance de base de la programmation Java.  
- JDK 8 ou une version plus récente installé sur votre machine.  
...
Tout d’abord, importez les classes dont nous aurons besoin. Elles comprennent les types graphiques AWT standard ainsi que l’API Aspose.Page.
...
Nous tracerons un cercle à l’aide de `Ellipse2D.Float`. Les paramètres sont `(x, y, largeur, hauteur)`. En définissant largeur = hauteur, on obtient un cercle parfait.
...
Maintenant que nous avons une forme, l’étape suivante consiste à **dessiner un cercle avec un dégradé**. En appliquant un `RadialGradientPaint` au cercle, l’opération de remplissage utilisera automatiquement le dégradé que nous définissons.
...
L’`AffineTransform` met à l’échelle et translate le dégradé pour l’adapter à notre cercle. La matrice `(scaleX, 0, 0, scaleY, translateX, translateY)` fait le travail.
...
Nous construisons maintenant l’objet `RadialGradientPaint`. Il prend le point central, le rayon, le point de focalisation, les fractions de couleur, le tableau de couleurs, la méthode de cycle, l’espace colorimétrique et la transformation que nous venons de définir.
...
## Étape 9 : Appliquer le Paint et remplir le cercle
Appliquez le paint dégradé au document et remplissez le cercle précédemment défini. C’est le cœur de notre **exemple de dégradé radial** et cela montre comment **remplir une forme avec un dégradé**.
...
Félicitations ! Vous avez créé avec succès un exemple de dégradé radial en Java PostScript à l’aide d’Aspose.Page. Vous disposez désormais d’un modèle réutilisable pour **remplir une forme avec un dégradé** qui peut être adapté à d’autres formes et formats de sortie.
...
| Le dégradé apparaît plat ou manquant | Assurez‑vous que le tableau `fractions` correspond à la longueur du tableau `colors` et que l’`AffineTransform` met à l’échelle correctement. |
...
R : Bien sûr, demandez‑en une depuis la [page de licence temporaire](https://purchase.aspose.com/temporary-license/).
...
## Conclusion
Dans ce guide, nous avons construit un **exemple complet de dégradé radial** pour un document PostScript à l’aide d’Aspose.Page pour Java. En suivant les étapes, vous disposez désormais d’un modèle réutilisable pour **remplir une forme avec un dégradé**, que vous pouvez adapter à PDF, SVG ou tout autre format pris en charge par Aspose.Page. Expérimentez avec différentes couleurs, rayons et formes pour enrichir vos projets graphiques Java.
