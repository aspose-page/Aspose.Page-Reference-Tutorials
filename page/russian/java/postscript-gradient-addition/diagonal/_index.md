---
date: 2026-02-13
description: Улучшите свои документы Java PostScript с помощью диагональных градиентов,
  используя Aspose.Page Java. Узнайте, как добавить градиентные эффекты с помощью
...
title: 'Как добавить градиент: диагональный градиент в Java PostScript с использованием
  Aspose.Page Java'
url: /ru/java/postscript-gradient-addition/diagonal/
weight: 10
---

GradientPaint`. Keep as is.

Also bullet lists with dash.
...
## Введение
Если вы хотите обогатить файл PostScript плавным диагональным переходом цветов, **Aspose.Page Java** делает это удивительно просто. В этом руководстве мы пошагово разберём, **как добавить градиент**, используя класс `LinearGradientPaint` из Java 2D. К концу вы получите готовый фрагмент кода, который создаёт документ PostScript с ярким диагональным градиентом.

## Как добавить градиент в Java PostScript
Добавление градиента может звучать как задача только для графики, но с Aspose.Page вы получаете полный контроль над базовыми командами PostScript, оставаясь в чистой Java. В этом разделе объясняется, почему такой подход работает и какие преимущества он даёт по сравнению с ручным написанием кода PostScript.
...
Aspose.Page Java — это мощный API, позволяющий разработчикам генерировать, редактировать и конвертировать файлы PostScript и PDF без необходимости в стороннем программном обеспечении. Он предоставляет полный набор графических возможностей языка PostScript через чистый объектно‑ориентированный интерфейс Java.
...
- **Aspose.Page for Java** — загрузите последнюю версию со [официальной страницы загрузки](https://releases.aspose.com/page/java/).
...
Определяем папку, в которой будет сохранён файл, и открываем `FileOutputStream`. Этот поток получит сгенерированные данные PostScript.
...
Создаём экземпляр `PsDocument`, передавая выходной поток и параметры сохранения. Флаг `false` указывает конструктору не открывать автоматически новую страницу — мы сделаем это позже.
...
`AffineTransform` позволяет вращать, масштабировать и перемещать градиент, чтобы он проходил по диагонали прямоугольника. Ниже‑приведённые расчёты вычисляют гипотенузу и корректируют коэффициент масштабирования.
...
## Шаг 6: Создание диагонального линейного градиента
Это основная часть **как добавить градиент** — мы создаём `LinearGradientPaint`, который простирается от верхнего‑левого до нижнего‑правого угла прямоугольника, используя ранее определённую трансформацию. `MultipleGradientPaint.CycleMethod.NO_CYCLE` гарантирует, что градиент не будет повторяться.
...
## Шаг 7: Установка краски и заполнение прямоугольника
Применяем градиентную краску к документу и заполняем форму прямоугольника. Этот шаг отрисовывает диагональный переход цветов на странице PostScript.
...
- **Градиент выглядит плоским** — проверьте угол вращения; угол 45° создаёт истинную диагональ.  
...
- **Ошибка «файл не найден»** — проверьте, что `dataDir` указывает на существующую папку и у приложения есть права записи.
...
О: Посетите сообщество‑форум [Aspose.Page forum](https://forum.aspose.com/c/page/39), где помогут инженеры Aspose и другие разработчики.
